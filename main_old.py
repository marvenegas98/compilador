...
class analizadorSintactico:
    ...
    def atom(self):
        ...
        if tok.tipo == TOK_TIPO:
            ...
            if self.tok_actual.tipo == TOK_PARENDER:
                ...
                return res.fallo(Error(self.tok_actual.pos_start, self.tok_actual.pos_end, self.tok_actual.tipo,"Se esperaba un ')'"))

            return res.fallo(Error(tok.pos_start, tok.pos_end,self.tok_actual.tipo,"Se esperaba un int, float, identifier, '+', '-' o '('"))
    ...
    def expr(self): #Clasificar como expresion
        res = Resultado()
        TOK_TIPO=self.tok_actual.tipo

        if (self.tok_actual.iguales(TOK_TIPO,'entero')) or (self.tok_actual.iguales(TOK_TIPO,'ent[]')) or (self.tok_actual.iguales(TOK_TIPO,'Cadena[]')) or (self.tok_actual.iguales(TOK_TIPO,'Cadena')) or (self.tok_actual.iguales(TOK_TIPO,'bool')):
            res.registrar(self.avanzar())

        elif self.tok_actual.tipo != TOK_IDENTIFICADOR:
            return res.fallo(Error(self.tok_actual.pos_start, self.tok_actual.pos_end,self.tok_actual.tipo,"Se esperaba un identificador"))

	nombre_var = self.tok_actual

        elif self.tok_actual.tipo != TOK_ASIG:
            return res.fallo(Error(self.tok_actual.pos_start, self.tok_actual.pos_end, self.tok_actual.tipo,"Se esperaba un '='"))
